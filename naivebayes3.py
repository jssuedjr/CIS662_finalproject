...
# block based method for a square array 
def downsample_to_28x28(array):
    # Get the size of the original array
    orig_size = array.shape[0]
    block_size = orig_size // 28  # Assuming the array is square and divisible by 28
    # Initialize the downsampled array
    downscaled = np.zeros((28, 28), dtype=np.uint8)
    ...
    for i in range(28):
        for j in range(28):
            ...
            # Sum the block and threshold it
            downscaled[i, j] = 1 if np.sum(block) > 0 else 0
